...
const STORAGE_KEY = 'kontana_state_v1';
    ...
    function openPaymentModal(walletId = null, mode = 'incoming') {
      ...
      app.paymentDraft.mode = mode === 'outgoing' ? 'outgoing' : 'incoming';
    ...
    function renderCashOnHand() {
      ...
      const rows = getSortedDenoms(wallet, 'largest_first')
        ...
        .filter((row) => {
          if (!app.state.settings.show_denominations_on_hand_only) return true;
    ...
    function renderNewPayment(target = 'tab-payment') {
      ...
      const insufficientOutgoing = strategiesEnabled && mode === 'outgoing' && ctx && ctx.amountMinor !== null && ctx.status === 'INSUFFICIENT_FUNDS';
      ...
      const allocationRows = rowsForTab
        .map((row) => {
          ...
          const willExceed = strategiesEnabled && amountMinor !== null && (effectiveAllocated + row.value_minor > amountMinor);
          ...
          return `<article class="denom-row ${willExceed ? 'denom-row-deemphasis' : ''}">
            ...
            <div class="denom-count-wrap">
              ...
              <input type="number" min="0" step="1" class="denom-input denom-input-strong" data-alloc-denom="${row.value_minor}" value="${count}" ${Number.isFinite(maxAvailable) ? `max="${maxAvailable}"` : ''} style="font-size:1.45rem;min-height:56px;font-weight:800;text-align:center;" />
              <button type="button" class="denom-step denom-step-strong ${disableAdd ? 'disabled' : ''}" data-alloc-step="1" data-denom="${row.value_minor}" ${disableAdd ? 'disabled' : ''} style="min-height:52px;height:52px;font-size:1.1rem;font-weight:800;">+</button>
            </div>
            <p class="denom-subtotal">${formatMoney(row.value_minor * count, wallet.currency)}</p>
          </article>`;
        }).join('');
              ...
              <section class="payment-breakdown-inline ${validAmount ? '' : 'disabled'}">
                ${validAmount ? `
                  ...
                  ` : `
                    ${!strategiesEnabled ? `
                      <div class="preview">
                        <p class="muted">Suggestions are disabled in Settings.</p>
                        <p class="muted">Enter denominations manually to continue.</p>
                      </div>
                    ` : `
                    <div class="preview ${suggestionToneClass}">
                      <p class="muted">Suggested</p>
                    ...
                    ` : ''}
                    <div class="preview ${mode === 'outgoing' && amountMinor !== null && effectiveAllocated > amountMinor ? 'suggestion-warn' : ''}">
                      ${mode === 'outgoing' && usingSuggestion ? `<p class="muted">Allocated (suggested): ${formatMoney(effectiveAllocated, wallet.currency)} / Expected: ${formatMoney(amountMinor || 0, wallet.currency)}</p>` : ''}
                      ${mode === 'outgoing' && usingSuggestion ? '' : `<p class="muted">Allocated: ${formatMoney(effectiveAllocated, wallet.currency)}${amountMinor !== null ? ` / Expected: ${formatMoney(amountMinor, wallet.currency)}` : ''}</p>`}
      ...
      if (showManualEditor) {
        el.querySelectorAll('input[data-alloc-denom]').forEach((input) => {
          input.addEventListener('input', (evt) => {
            ...
            app.paymentDraft.allocation[denom] = Math.min(next, maxAvail);
            app.paymentDraft.manualEntry = true;
            rerenderPayment();
          });
        });
        el.querySelectorAll('button[data-alloc-step]').forEach((btn) => {
          btn.addEventListener('click', () => {
            ...
            const maxAvail = mode === 'outgoing'
              ? (wallet.denominations.find((d) => d.value_minor === denom)?.count || 0)
              : Infinity;
            app.paymentDraft.allocation[denom] = Math.max(0, Math.min(current + step, maxAvail));
            app.paymentDraft.manualEntry = true;
            rerenderPayment();
          });
        });
    ...
    function renderTransactions() {
      ...
      const rows = filtered.map((tx) => {
        ...
        const walletLabel = wallet ? wallet.name : `${tx.currency} — ${tx.wallet_name}`;
        ...
        const notePreview = truncateNote(tx.note || '', 30) || '-';
        const changeSummary = tx.change_expected_minor
          ...
          : '-';
        const reasonText = tx.note || '-';
      const createdLabel = formatDateTimeEU(tx.created_at);
      const showCreated = false;

        return `
          <tr class="tx-row ${amountClass}">
            <td class="tx-date">${formatDateTimeEU(tx.created_at)}</td>
            <td class="tx-wallet">${walletLabel}</td>
            <td class="tx-amount ${amountClass}">${amountLabel}${formatMoney(amountMinor, tx.currency)}</td>
            <td class="tx-note">${notePreview}</td>
            <td class="tx-actions">
          ...
          <tr class="tx-details-row">
            <td colspan="5">
              <div class="tx-details-body">
                ...
                ${showCreated ? `<div class="tx-detail"><span>Created</span><span>${createdLabel}</span></div>` : ''}
      ...
      document.querySelectorAll('.tx-details summary').forEach((summary) => {
        summary.addEventListener('click', (e) => {
          const details = summary.closest('.tx-details');
          if (!details) return;
    ...
    function renderSettings() {
      ...
      const strategiesEnabled = app.state.settings.payment_strategies;
      const changeSuggestionsEnabled = app.state.settings.change_suggestions;
      const onHandOnly = app.state.settings.show_denominations_on_hand_only;
      ...
      el.innerHTML = `
        <section class="panel">
          ...
          <section class="subpanel">
            ...
            <p class="muted">Show only denominations you currently have.</p>
            <div class="segmented-control" role="group" aria-label="Denomination visibility">
              <button type="button" class="segment ${onHandOnly ? '' : 'active'}" data-denoms-visible="off">Off</button>
              <button type="button" class="segment ${onHandOnly ? 'active' : ''}" data-denoms-visible="on">On</button>
            </div>
          </section>
      ...
      el.querySelectorAll('button[data-denoms-visible]').forEach((btn) => {
        btn.addEventListener('click', () => {
          const next = btn.dataset.denomsVisible === 'on';
          if (app.state.settings.show_denominations_on_hand_only === next) return;
          app.state.settings.show_denominations_on_hand_only = next;
          saveState();
          render();
        });
      });
